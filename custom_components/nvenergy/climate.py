...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    for thermostat_id in thermostat_ids:
        if len(thermostat_ids) == 1:
            ...
            tnameid = "name_" + str(thermostat_id)
            
        thermostat_name = None

        if tnameid in config and len(config[tnameid]) > 0:
            thermostat_name = config[tnameid]

        simple_thermostat = client.createThermostat(thermostat_id)
        nve_thermostat = NVEThermostat(simple_thermostat, thermostat_name)
